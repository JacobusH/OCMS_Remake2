			...
			<div class="tablepart">
				...
				<table width="100%" border="0" cellspacing="1" cellpadding="0">
					<tr>
						...
						<th width="72%"><a href="<?=site_url(array($this->uri->segment(1), $this->url, 'sortby', 'state_name', 'disType', $this->disType1, 'pg', $this->pg ))?>">State Name</a></th>
						<th width="15%"><a href="<?=site_url(array($this->uri->segment(1), $this->url, 'sortby', 'state_code', 'disType', $this->disType1, 'pg', $this->pg ))?>">State Code</a></th>
						<?=$this->navigation->sub_actions_title(10)?>
					</tr>
					<?foreach($result as $key => $row){
					  $this->bgClass = ($key % 2) ? 'odd' : 'even';?>					
					<tr class="<?=$this->bgClass?>">
						<td align="center"><input type="checkbox" name="check" id="check" value="<?=$row->state_id?>"/></td>
						<td align="left"><?=$row->state_name?></td>
						<td align="center"><?=$row->state_code?></td>
						<?=$this->navigation->sub_actions($row->state_id, $row->status)?>
					</tr>
					<?}?>
				</table>
				<?}else{?>
		...
		<div class="rightpart">
			...
			<div class="tablepart">
				...
				<table border="0" cellspacing="0" cellpadding="0" class="middle">					
					<tr>
					  <td width="13%" align="left">State Name</td>
					  <td width="2%" align="center">:</td>
					  <td width="85%" align="left"><input type="text" class="form1" title="State Name" name="state_name" id="state_name" style="width: 180px;" value="<?=$row['state_name']?>"></td>
					</tr>
					<tr>
					  <td align="left">State Code</td>
					  <td align="center">:</td>
					  <td align="left"><input type="text" class="form1" name="state_code" id="state_code" title="State Code" style="width: 35px;" maxlength="3" value="<?=$row['state_code']?>"></td>
					</tr>
				</table>				
			</div>
		</div>
