			...
			<div class="tablepart">
				...
				<table width="100%" border="0" cellspacing="1" cellpadding="0">
					<tr>
						...
						<th width="72%"><a href="<?=site_url(array($this->uri->segment(1), $this->url, 'sortby', 'file_name', 'disType', $this->disType1, 'pg', $this->pg ))?>">Document Title</a></th>						
						<?=$this->navigation->sub_actions_title(10)?>
					</tr>
					<?foreach($result as $key => $row){
					  $this->bgClass = ($key % 2) ? 'odd' : 'even';?>					
					<tr class="<?=$this->bgClass?>">
						<td align="center"><input type="checkbox" name="check" id="check" value="<?=$row->img_id?>"/></td>
						<td align="left"><?=$row->file_name?></td>
						<?=$this->navigation->sub_actions($row->img_id, $row->status)?>
					</tr>
					<?}?>
				</table>
				<?}else{?>
			...
			<div class="tablepart">
				...
				<table border="0" cellspacing="0" cellpadding="0" class="middle">					
					<!-- <tr>
					  ...
					  <td width="85%" align="left"><input type="text" class="form1" title="Document Title" name="doc_title" id="doc_title" style="width: 180px;" value="<?=$row['file_name']?>"></td>
					</tr> -->
					<tr>
					  <td align="left">Upload</td>
					  <td align="center">:</td>
					  <td align="left" style="padding-top:10px;"><input type="file" name="file_name" id="file_name" class="form1" value="<?=$row['file_name']?>">
					  <span style="color:#970505;font-weight:bold;"><?=$row['file_name']?></span><input type="hidden" name="filename" id="filename" value="<?=$row['file_name']?>"><span style="color:red;font-size:13px;"><b>[800 X 217]</b></span>
					  <p style="padding:10px 0px 0px 8px;"><span style="color:red;font-size:13px;"><b>[Upload .gif |.jpg |.png |.bmp only]</b></span></p>
					  </td>
				    </tr>
				</table>				
			</div>
